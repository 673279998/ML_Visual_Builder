...
class MLPRegressor(BaseAlgorithm):
    """多层感知机回归算法(神经网络)"""
    
    def __init__(self):
        super().__init__()
        self.algorithm_name = "mlp_regressor"
    ...
    def train(self, X: np.ndarray, y: np.ndarray, **params) -> 'MLPRegressor':
        """
        训练模型
        ...
        # 创建并训练模型
        self.model = SKMLPRegressor(**validated_params)
        self.model.fit(X, y)
